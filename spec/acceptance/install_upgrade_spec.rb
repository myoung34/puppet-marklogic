    ...
    it 'should work with no errors' do
      pp = <<-EOS
        ...
        class { 'marklogic':
          ...
          disable_ec2_detection  => true,    #defaults to false
          is_development_license => true,    #defaults to false
          is_upgrade             => true,   #defaults to false
          licensee               => "#{ENV['MARKLOGIC_LICENSEE']}",#required
          license_key            => "#{ENV['MARKLOGIC_KEY']}", #required
      ...
      shell("rpm -qa | grep MarkLogic") do |result|
        assert_match /^MarkLogic-6.0-2.3.x86_64/, result.stdout, 'Upgrade was not successful.'
      end
    end
  end

  describe 'upgrade 6.0-4' do
    it 'should work with no errors' do
      pp = <<-EOS
        yumrepo { "marklogic":
          baseurl         => "#{ENV['MARKLOGIC_YUM_URL']}",
      ...
      shell("rpm -qa | grep MarkLogic") do |result|
        assert_match /^MarkLogic-6.0-4.x86_64/, result.stdout, 'Upgrade was not successful.'
      end
    end
  end

  describe 'upgrade 6.0-4.1' do
    it 'should work with no errors' do
      pp = <<-EOS
        yumrepo { "marklogic":
          baseurl         => "#{ENV['MARKLOGIC_YUM_URL']}",
      ...
      shell("rpm -qa | grep MarkLogic") do |result|
        assert_match /^MarkLogic-6.0-4.1.x86_64/, result.stdout, 'Upgrade was not successful.'
      end
    end
  end

  describe 'upgrade 7.0-1' do
    it 'should work with no errors' do
      pp = <<-EOS
        yumrepo { "marklogic":
          baseurl         => "#{ENV['MARKLOGIC_YUM_URL']}",
      ...
      shell("rpm -qa | grep MarkLogic") do |result|
        assert_match /^MarkLogic-7.0-1.x86_64/, result.stdout, 'Upgrade was not successful.'
      end
    end
  end
end
